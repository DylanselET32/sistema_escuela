...
function updateCalificacion($id,$id_alumno,$materia,$cuatrimestre,$nota,$fecha){
    global $conn;
    $query = "UPDATE `calificacion` SET `id_alumno`= $id_alumno,`materia`='$materia',`cuatrimestre`=$cuatrimestre,`nota`=$nota,`fecha`= '$fecha' WHERE  id = $id";
    $result = mysqli_query($conn,$query);
    if(!$result){
        return "Error SQL" ;   
    }
...
function getAllCalificacionesWithAlumnos(){
    global $conn;
    $query = "SELECT * FROM calificacion as c  INNER JOIN alumno as a ON a.id = c.id_alumno order by a.nombre, a.apellido , c.fecha asc;";
    $result = mysqli_query($conn,$query);
    if(!$result){
        return "Error SQL" ;   
    }
...
function getCalificacionWithAlumnoById($id){
    global $conn;
    $query = "SELECT * FROM alumno as a INNER JOIN calificacion as c ON a.id = c.id_alumno where c.id = $id;";
    $result = mysqli_query($conn,$query);
    if(!$result){
        return "Error SQL" ;   
    }
